...
if __name__ == "__main__":
    ...
    parser.add_argument('--goal_file_path', dest='goal_file_path', type=str,
                        default='deep_dialog/data/user_goals_all_turns_template.part.movie.v1.p', help='a list of user goals')
    ...
    parser.add_argument('--intent_err_prob', dest='intent_err_prob',
                        default=0.05, type=float, help='the intent err probability')
    ...
    parser.add_argument('--act_level', dest='act_level', type=int,
                        default=0, help='0 for dia_act level; 1 for NL level')
    ...
    parser.add_argument('--auto_suggest', dest='auto_suggest', type=int,
                        default=0, help='0 for no auto_suggest; 1 for auto_suggest')
...
if usr == 0:  # real user
    user_sim = RealUser(movie_dictionary, act_set, slot_set, usersim_params)
elif usr == 1:
            ...
            if params['trained_model_path'] == None and selfplay_params['exploration'] == 'True':
                if ((episode + 1) % selfplay_params['update_step'] == 0):
                    if exploration_params['additive'] == 'True':
                        factor = int(episode / selfplay_params['update_step'])
                        selfPlay.prob_update_epsilon_increment(
                            theta_1=exploration_params['theta_1']*factor, theta_2=exploration_params['theta_2']*factor, theta_3=exploration_params['theta_3']*factor, additive=True)
                    else:
                        selfPlay.prob_update_epsilon_increment(
